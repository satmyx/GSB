...
class PDF extends tFPDF {
    ...
    function getSignatureComptable() {
        $imgSignatureComptable = PATH_OUTILS . '/signatureComptable.png';
        $this->Image($imgSignatureComptable, 150, 270, 50);
    }
...
$moisList = array('Janvier', 'Février', 'Mars', 'Avril', 'Mai', 'Juin', 'Juillet', 'Août', 'Septembre', 'Octobre', 'Novembre', 'Décembre');
...
$moisAfficheFR = $moisList[$mois -1];
$annee = substr($row['mois'], 0, 4);

$pdf->Text(15, 88, 'Mois : ' . utf8_decode($moisAfficheFR) . ' ' . $annee);
...
while ($row3 = mysqli_fetch_array($rep3)) {
    ...
    $pdf->SetX(37);
    $pdf->MultiCell(45, 10, utf8_decode(implode('/',array_reverse(explode('-', $row3['date'])))), 1, 'C');
    $pdf->SetY($position_detail2);
    $pdf->SetX(82);
    ...
    if ($pos !== FALSE) {
        $refusMontant = 0;
        $pdf->MultiCell(45, 10, $refusMontant, 1, 'C');
...
$pdf->Ln();
$pdf->SetX(127);
$pdf->Cell(50, 10, utf8_decode('Fait à Toulon le ') . utf8_decode($formatter->format(time())));
$pdf->Ln();
$pdf->SetX(122);
$pdf->Cell(50, 10, utf8_decode('Vu l\'agent comptable '), 0, 0, 'C');
$pdf->Ln();
$pdf->SetX(125);
